...
import moe.eliotvonecklie.immersive.blockentities.SecurityFactionRankReaderBlockEntity;
...
public class SecurityFactionRankReader extends Block implements BlockEntityProvider {
	public static final DirectionProperty FACING = Properties.HORIZONTAL_FACING;
	public static final BooleanProperty POWERED = Properties.POWERED;
	public static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;
	public static final BooleanProperty CONFIGURED = BooleanProperty.of("configured");

	public SecurityFactionRankReader(Settings settings) {
		super(settings);
	...
	public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		if (!world.isClient()) {
			SecurityFactionRankReaderBlockEntity be = (SecurityFactionRankReaderBlockEntity) world.getBlockEntity(pos);
			
			User playerUser = User.get(player.getUuid());
			
			if (playerUser.getFaction() == null) {
				player.sendMessage(Text.literal("Not in a faction."), false);
				return ActionResult.FAIL;
			}
			
			if (!state.get(CONFIGURED)) {
				if (player.isSneaking()) {
					be.configure(playerUser);
				...
				player.sendMessage(Text.literal("Not configured, use while sneaking to configure it with your rank."), false);
				
				return ActionResult.FAIL;
			}
			
			if (new FactionOwner(playerUser).owns(be)) {
	...
	*/
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new SecurityFactionRankReaderBlockEntity(pos, state);
    }
